...
st.write('For each voxel $\small v$ in a tensorial image $\small f$, the dissimilarity measure $\small (d_n)$ between any pair of tensors $\small (T_i,T_j)$ in its neighborhood is calculated. The neighborhood is defined\
          by a structuring element $\small B$. The computed gradient will be the maximum dissimilarity among all pairwise dissimilarities:')
...
st.write('There are several dissimilarity measures proposed for DTI applications and explored through the TMG [[1]](#references), some of which are presented below. The choice of the structuring element generally depends on the\
          ...
          Additional information about the metrics and examples of TMG computation using synthetic data can be found in [TMG Metrics Info and Examples on Synthetic Data.ipynb](https://github.com/MICLab-Unicamp/TMG-based_CC_Parcellation/tree/main/examples_TMG/TMG%20Metrics%20Info%20and%20Examples%20on%20Synthetic%20Data.ipynb).\
          To visualize different SE options, use [TMG Neighborhood Visualization.ipynb](https://github.com/MICLab-Unicamp/TMG-based_CC_Parcellation/tree/main/examples_TMG/TMG%20Neighborhood%20Visualization.ipynb)\
...
st.write('The Dot Product [[2]](#references) only considers the principal eigenvectors of the tensors. The result varies between 0 and 1 according to the angle between the\
          vectors: it is equal to one for parallel vectors and equal to zero for orthogonal vectors.')
...
st.write('Since the Dot Product is a similarity measure, the TMG calculates the minimal similarity instead of the maximal dissimilarity. The negative of the resulting image\
          is then used as the final TMG output, maintaining consistency with the other measures.')

#-----------------

st.subheader('Frobenius Norm (frob)')

st.write('The Frobenius Norm [[3]](#references) is a Euclidean distance measure between the tensors, as follows:')
...
st.write('The J-divergence [[4]](#references) is a tensorial dissimilarity measure based on concepts from information theory. The measure is not a true distance, since it violates the triangle\
...
st.markdown("""with $\small n$ being the size of the square matrix $\small T$.""")

#-----------------

st.subheader('Log-Euclidean distance (logE)')

st.write('The logE [[5]](#references) is an Euclidean distance measure between the logarithms of the tensors, closely related to the Frobenius Norm.')
...
st.write('From the equation above, it is possible to see that symmetric matrices with null or negative eigenvalues are at an infinite distance from any tensor.\
          To overcome this problem, $\small \log(T_i)$ was replaced with $\small E_i \cdot \log(100\lambda_i+1)I\cdot E_i^T$, where $\small E$ corresponds to the matrix of eigenvectors,\
          $\small \lambda$ corresponds to the eigenvalues, and $\small I$ is the identity matrix.')
...
st.markdown("""[5] Arsigny, V., Fillard, P., Pennec, X., Ayache, N. (2006). Log‐Euclidean metrics for fast and simple calculus on diffusion tensors. _Magnetic Resonance in Medicine_, doi: \
