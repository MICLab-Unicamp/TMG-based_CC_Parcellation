...
st.write('The TMG [[1]](#references) uses the dissimilarity between neighboring voxels to summarize the main tensorial information into a scalar map. It adapts the idea of a morphological gradient to tensorial images and\
          was proposed to facilitate segmentation and parcellation tasks using Diffusion Tensor Imaging (DTI) data.')
...
st.write('There are several dissimilarity measures proposed for DTI applications and explored through the TMG [[1]](#references). The choice of the structuring element generally depends on the information\
          required (bi- or three-dimensional) and the size of the studied structure - smaller structuring elements tend to generate thinner, more detailed borders.')
...
st.write('The Dot Product [[2]](#references) only takes in consideration the main eigenvectors. The result varies between 0 and 1 according to the angle between the vectors: equals to one if the vectors are\
...
st.write('This dissimilarity measure is not invariant to affine transformations of rotation. However, it presents a linear response to differences in anisotropy and trace.')

#-----------------

st.subheader('J-Divergence (Jdiv)')

st.write('The J-divergence [[4]](#references) is a tensorial "distance" measure, based on concepts of theory information. The measure is not truly a distance, since it violates the triangular inequality. However, \
         it is a computationally efficient approximation of the Rao distance and is invariant to affine transformations.')
...
st.markdown("""[2] Ziyan, U., Tuch, D., Westin, C. F. (2006). Segmentation of thalamic nuclei from DTI using spectral clustering. In _Medical Image Computing and Computer-Assisted Intervention–MICCAI 2006_, doi: \
            [10.1007/11866763_99](https://doi.org/10.1007/11866763_99).""")
